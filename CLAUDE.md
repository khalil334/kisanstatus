...
## Article fix progress — components/articles/hindi-yojana/

Rule: 2000+ words, no repetitive/AI/uniform phrasing, unique structure per article,
farmer-useful + traffic keywords, no invented facts/numbers.
...
| 14 | TractorSubsidy2027StateWiseList | **done** — ~2100 words, 4-reject-reasons section (naam-mismatch / non-empanelled dealer / deadline / NPCI seeding), dealer-se-4-sawaal pre-purchase checklist, re-apply + lock-in-period FAQ (9 total), state-wise portals kept, no invented percentages |
| 15 | PmKisanHelplineNumberComplaint | **done** — ~2290 words, 10 H2, problem→sahi-rasta triage table, phone/online/email 3 raste, email format template, 4-galtiyan query-writing section, CPGRAMS escalation route (pgportal.gov.in), Nodal Officer path, asli payment-failed case, 12 FAQ (CSC, multiple queries, no-network village), no invented numbers/dates |
