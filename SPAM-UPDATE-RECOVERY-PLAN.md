...
| **3** | **Thin pages** — GSC "Crawled – not indexed" list ke pages: expand (2,000+ words real info) ya nearest keeper me merge + 301. GSC ab API se connected hai — export ki zaroorat nahi, list URL-inspection se nikaali jayegi. Sub-parts 3a–3f neeche | 🟡 in progress (25 Aug) — GSC API re-connected (siteOwner verify), live sitemap pull ho gaya (133 URLs); 3a sweep next |
| **3a** | **Index-status sweep** — live sitemap ke 133 URLs URL Inspection API se sweep karo (2,000/day quota ke andar); har URL ka verdict/coverage/canonical/last-crawl record karo | 🟡 in progress |
| **3b** | **Thin-list classify** — sweep results se "Crawled – currently not indexed" + "Discovered – currently not indexed" pages nikaalo; har ek ko classify karo: expand (2,000+ words real info) ya nearest keeper me merge + 301. Protect-list pages ko haath nahi lagana | ⏸️ pending (3a ke baad) |
| **3c** | **Merge batch** — jo pages merge+301 classify hue: data entries + components delete, 301s next.config.js me, cross-links keeper pe repoint, hreflang clean (Part 2b–2c wala pattern) | ⏸️ pending |
| **3d** | **Expand batch 1** — expand-classify pages ka pehla aadha: har page pe 2,000+ words asli, page-specific info (koi template filler nahi), Sources + freshness line consistent | ⏸️ pending |
| **3e** | **Expand batch 2** — baaki expand pages, same standard | ⏸️ pending |
| **3f** | **Verify + wrap** — `tsc --noEmit` + production build clean, removed slugs ka repo-wide grep zero, plan file final update | ⏸️ pending |
