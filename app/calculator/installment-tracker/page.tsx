...
export const metadata: Metadata = {
  ...
  keywords: ['pm kisan installment tracker', 'pm kisan 23vi kist status'],
...
export default function Page() {
  ...
  if (ekyc === 'no') blockers.push({ icon: '🔐', title: 'eKYC Pending', desc: 'Bina eKYC ke kist nahi aayegi.', href: '/articles/pm-kisan-ekyc-online-2026', cta: 'eKYC Guide' });
  if (seeding === 'no') blockers.push({ icon: '🏦', title: 'Bank Seeding Pending', desc: 'NPCI seeding zaroori hai.', href: '/articles/pm-kisan-payment-failed-status-2026', cta: 'Payment Guide' });
  if (land === 'no') blockers.push({ icon: '🌾', title: 'Land Seeding No', desc: 'Zameen link nahi hui.', href: '/articles/pm-kisan-land-seeding-status-check', cta: 'Land Guide' });
  if (status === 'rejected') blockers.push({ icon: '❌', title: 'Status Rejected', desc: 'Rejection reason check karo.', href: '/articles/pm-kisan-rejected-list-2026', cta: 'Rejected Guide' });
    ...
    <>
      <div className="bg-primary-600 py-8">
        <div className="container-site max-w-3xl">
          ...
          <h1 className="text-2xl md:text-3xl font-black text-white leading-tight mb-3">PM Kisan Installment Status Tracker 2026</h1>
      ...
      <div className="container-site max-w-2xl py-8">
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl text-sm text-gray-700">
          <p><strong>📆 {INSTALLMENT_NAME}</strong> 20 June 2026 ko release ho chuki hai — <strong>9.44 crore+</strong> farmers ko ₹2,000 mil chuke hain. Agar tumhare account mein abhi tak nahi aaya, neeche 4 sawaalon ke jawab do.</p>
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h2 className="font-black text-gray-900 text-base mb-5 flex items-center gap-2"><span>🧮</span> Apna Status Check Karo</h2>
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-800">
            ...
            <a href="https://pmkisan.gov.in/BeneficiaryStatus.aspx" target="_blank" rel="noopener noreferrer" className="underline font-bold">pmkisan.gov.in par check karo</a>, phir neeche jawab do.
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">eKYC complete hai?</label>
              <select value={ekyc} onChange={e => setEkyc(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-white">
            ...
            <div>
              ...
              <select value={seeding} onChange={e => setSeeding(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-white">
            ...
            <div>
              ...
              <select value={land} onChange={e => setLand(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-white">
            ...
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-white">
          ...
          {!allAnswered ? (
            <div className="mt-6 p-5 bg-gray-50 border-2 border-gray-300 rounded-2xl text-center">
              ...
              <p className="text-sm text-gray-600">Upar 4 sawaalon mein se kuch ka jawab "Pata nahi" hai. Pehle apna status check karo.</p>
              <a href="https://pmkisan.gov.in/BeneficiaryStatus.aspx" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-blue-500 transition-colors mt-3">🏛️ Pehle Official Status Check Karo →</a>
            </div>
          ) : allGood ? (
            <div className="mt-6 p-5 bg-green-50 border-2 border-green-300 rounded-2xl text-center">
              ...
              <p className="text-sm text-green-700 mb-3">eKYC, bank seeding, land seeding aur status — sab theek hai.</p>
              <a href="https://pmkisan.gov.in/BeneficiaryStatus.aspx" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-green-700 text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-green-600 transition-colors">🏛️ Official Status Check Karo →</a>
                  ...
                  <div key={b.title} className="bg-white border border-orange-200 rounded-xl p-3.5">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1">
                        ...
                        <Link href={b.href} className="inline-flex items-center gap-1 text-xs font-bold text-orange-700 hover:text-orange-900">{b.cta} →</Link>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-center gap-3">
          ...
          <div><p className="font-bold text-blue-900 text-sm">Agli Kist</p><p className="text-xs text-blue-700">24वीं किस्त — October 2026 (Expected)</p></div>
        </div>
