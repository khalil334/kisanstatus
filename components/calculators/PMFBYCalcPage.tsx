...
export default function PMFBYCalcPage() {
  ...
  const schema = {
    ...
    name:'PMFBY Fasal Bima Premium Calculator 2026',
    url:'https://kisanstatus.com/calculator/pmfby-premium',
    applicationCategory:'FinanceApplication',
    description:'Pradhan Mantri Fasal Bima Yojana premium calculator — fasal bima kitna lagega',
  ...
  return (
    <>
      ...
      <CalcHeader
        emoji="🛡️"
        title="PMFBY Fasal Bima Premium Calculator 2026"
        subtitle="Apni fasal ka insurance premium calculate karo — Kharif aur Rabi dono ke liye"
        breadcrumb="PMFBY Premium"
      />
      ...
      <div className="container-site max-w-2xl py-8">
        ...
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-gray-700">
          <p><strong>🛡️ PMFBY (Pradhan Mantri Fasal Bima Yojana)</strong> mein fasal kharab hone par government claim deti hai. Premium bahut kam hai — <strong>sirf 1.5% to 5%</strong> aapko dena hota hai, baaki government bharta hai.</p>
        </div>
          ...
          {insured>0 && (
            <div className="mt-4 bg-amber-50 border-2 border-amber-200 rounded-2xl p-5">
              <p className="text-xs text-amber-700 font-bold uppercase tracking-wide mb-3">
                📊 Aapka Fasal Bima Premium — {cropData.name} ({season==='kharif'?'Kharif':'Rabi'})
              </p>
              ...
              <div className="mt-3 pt-3 border-t border-amber-200">
                <p className="text-xs text-amber-800">
                  🛡️ Fasal kharab hone par maximum claim: <strong>{fmt(totalCover)}</strong>
                </p>
              </div>
            </div>
          )}

          {/* Claim process quick guide */}
          <div className="mt-5 p-4 bg-green-50 border border-green-200 rounded-xl">
            <p className="font-bold text-green-900 text-xs mb-2">⚡ Fasal Kharab Ho Jaaye To — Claim Kaise Karein?</p>
            <div className="space-y-1.5 text-xs text-gray-700">
              {[
                '72 ghante mein bank ya insurance company ko inform karo',
                'Crop loss ki photos aur video lo — proof ke liye',
                'Kisan portal ya PMFBY app par claim file karo',
                'District Agriculture Officer ko written complaint do',
                'Survey ke baad 2-4 hafte mein claim settle hota hai',
              ].map((s,i)=>(
        ...
        <div className="mt-6 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          <h3 className="font-black text-gray-900 text-sm mb-3">✅ PMFBY Ke Liye Eligible Koun Hai?</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
            {[
              {t:'All farmers',d:'Land owner aur tenant farmers dono'},
              {t:'Sabhi crops',d:'Kharif, Rabi, horticulture — sab cover'},
              {t:'KCC wale',   d:'Compulsory enrollment for KCC holders'},
              {t:'Non-KCC',    d:'Optional — khud apply kar sakte ho'},
            ].map(({t,d})=>(
          ...
          <Link href="/articles/pmfby-crop-insurance-2026"
            ...
            📖 PMFBY Complete Guide Padho →
          </Link>
        </div>

        <CalcDisclaimer note="PMFBY premium rates district-wise alag hote hain. Exact figures ke liye apne bank, CSC center ya PMFBY portal se confirm karein." />
