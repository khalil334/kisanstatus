      ...
      <div className="container-site max-w-3xl py-8">
        ...
        <section className="mb-8">
          ...
          <div className="space-y-3">
            {[
              { n: 1, reason: 'Digital Verification Incomplete', desc: '60% cases mein yahi hota hai. UID OTP authentication nahi hui.', fix: 'pmkisan.gov.in → verification → UID → OTP verify. Free, 5 minute.' },
              { n: 2, reason: 'Land Record Linking Nahi', desc: '"Land Seeding No" dikhta hai. Zameen portal se link nahi.', fix: 'Patwari se Khasra-Khatauni update → BAO application.' },
              { n: 3, reason: 'Naam Mismatch', desc: 'UID mein naam alag, bank mein alag. System decline kar deta hai.', fix: 'Farmers Corner → Name Correction → UID jaisa naam daalo.' },
              { n: 4, reason: 'NPCI Mapper Missing', desc: 'Bank account mein UID link nahi. NPCI seeding missing.', fix: 'Bank branch → Aadhaar seeding form → 3-7 din fix.' },
              { n: 5, reason: 'Duplicate Enrollment', desc: 'Same UID se do baar enrollment ho gaya.', fix: 'Helpline 155261 par call → duplicate entry remove karwao.' },
              { n: 6, reason: 'Ineligible Category', desc: 'Govt employee, income tax filer, MP/MLA — eligible nahi.', fix: 'Eligibility dobara check. Galat decline = Agriculture office appeal.' },
              { n: 7, reason: 'State Verification Pending', desc: 'Enrollment hua lekin state ne abhi verify nahi kiya.', fix: '2-4 hafte wait. Phir bhi nahi = Block Agriculture Officer contact.' },
