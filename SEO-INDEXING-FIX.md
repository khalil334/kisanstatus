...
**Revision 4 (2026-08-17):** post-deploy — all 13 parts verified LIVE; fresh GSC inspection (1 page newly indexed, 2 out of 'unknown'); NEW §C: GSC coverage errors (404/redirects/soft-404) + fixes
...
| **13** | Content upgrade on the 2 "crawled — currently not indexed" PMKMY pages | P2 | ✅ **done 2026-08-17** — contextual in-body links added INTO both pages from 4 sibling maandhan articles (status-check ×2, registration, auto-debit, withdrawal-refund); both pages upgraded: bank-account-change intro now cross-references the auto-debit NACH mechanics + regularization guides and Problem 5 links the grievance escalation format; grievance-helpline gained a final CPGRAMS (pgportal.gov.in) escalation step and decision-tree links to bank-change + regularization. No new factual claims added — only cross-references and the public CPGRAMS portal. Verified in the webpack production build. Remaining manual step: **Request Indexing** on both URLs in GSC after deploy. |
...
# REVISION 4 — Post-deploy verification + GSC coverage errors (2026-08-17)


---

## A. Deploy verification — all 13 parts confirmed LIVE ✅

Checked by fetching the live pages (not the repo):

| Check | Live result |
|---|---|
| 10 severe titles realigned (Fix 2) | ✅ Sample 9/9 correct — e.g. `/articles/KisanRinKahaSeLe2026` title is now *"Kisan Rin Kaha Se Le 2026 — KCC, Bank, CSC"*; maandhan-withdrawal is *"Maandhan Exit: Refund & Family Pension Ka Sach"*; `/about`, `/contact`, msp-list, gau-mutra, state-list, pmkmy-bank-change all match their H1 |
| Over-length titles (Fix 6) | ✅ `/disclaimer` 43 chars, pashupalan category 56 chars |
| `/search` robots conflict (Fix 4) | ✅ robots.txt no longer Disallows `/search`; `noindex` retained |
| `/search` self-canonical (Fix 7) | ✅ canonical = `https://kisanstatus.com/search` |
| Homepage hub nav (Part 9) | ✅ "Sabhi Sections Ek Nazar Mein" + all 7 calculator pills + maandhan/rajya-yojana cards |
| Footer navigation (Part 10) | ✅ 6 English + 6 Hindi category pages linked site-wide |
| Cross-section links (Part 11) | ✅ Sample article carries 16 unique cross-section links |
| hreflang pairs (Part 12) | ✅ alternate tags rendering |
| Sitemap resubmit (P0) | ✅ Resubmitted 2026-08-17 20:29 UTC, Google downloaded it seconds later — 0 errors, 0 warnings, 138 web URLs + 108 images |

## B. Fresh URL Inspection — first movement already visible

Pulled from the GSC URL Inspection API on 2026-08-17, after deploy:

| URL | Was (§2B) | Now | Movement |
|---|---|---|---|
| `/maandhan/pmkmy-bank-account-change` | Crawled — not indexed | **Submitted and indexed** ✅ | **FIXED** |
| `/maandhan` | URL unknown to Google | Discovered — not indexed | ✅ progress (now attributed to sitemap) |
| `/articles/hi/state-kisan-yojana-list` | URL unknown to Google | Discovered — not indexed | ✅ progress |
| `/articles/hi` | Discovered — not indexed | Discovered — not indexed | awaiting crawl |
| `/calculator/crop-profit` | Discovered — not indexed | Discovered — not indexed | awaiting crawl |
| `/articles/hi/rythu-bharosa-status` | Discovered — not indexed | Discovered — not indexed | awaiting crawl |
| `/rajya-yojana` | URL unknown to Google | **URL unknown to Google** ❌ | no movement yet |
| `/maandhan/pmkmy-grievance-complaint-helpline` | Crawled — not indexed | Crawled — not indexed (last crawl 2026-07-29 — Google hasn't seen the Part-13 content upgrade yet) | needs Request Indexing |

**Interpretation:** the internal-linking fixes are starting to work (two URLs moved out of
"unknown", one page got indexed). The "Discovered" bucket clears only when Googlebot actually
crawls — expect movement over 2–4 weeks. No further code changes needed for these.

### Immediate manual actions (GSC UI, ~10 minutes)

1. **Request Indexing** on, in this order:
   - `https://kisanstatus.com/rajya-yojana` (still unknown — highest priority)
   - `https://kisanstatus.com/maandhan`
   - `https://kisanstatus.com/articles/hi`
   - `https://kisanstatus.com/calculator`
   - `https://kisanstatus.com/maandhan/pmkmy-grievance-complaint-helpline` (so Google re-crawls the Part-13 content upgrade)
   - `https://kisanstatus.com/` (homepage — picks up the new hub nav fastest)
2. **Trigger the Ahrefs Site Audit re-crawl** (project `10042735`) — issue
   `d69246c2-225a-11ec-8456-06d2f2f613d8` ("Page and SERP titles do not match") should drop from 33.

---

## C. GSC coverage errors NOT covered by the original document

The GSC coverage export (data through 2026-08-14) lists these buckets. The original document's
crawl found all 138 sitemap URLs returning `200 OK` with 0 redirects — so **every URL below is
outside the current sitemap**: old slugs, deleted pages, or URLs Google remembers from before.
None of them block the indexing recovery, but they waste crawl budget and two validations have
already FAILED, so they should be closed out.

**Prerequisite for all fixes below:** export the exact URL list per bucket from GSC
(Pages → click the reason → Export). The fixes are written per-bucket; the URL lists decide
which rule each URL gets. **Do not guess redirect targets — map each old URL to its real
replacement, or return 410.**

### C.1 Page with redirect — 12 pages, validation FAILED ❌

**What it means:** Google requested these URLs and got a 3xx. GSC lists them as "not indexed"
because the redirect target gets indexed instead. **This is usually correct behaviour** — the
validation "fails" only because the old URLs still exist as redirects (they never become
indexable, so the validation can never pass).

**Fix:**
1. Export the 12 URLs. For each, confirm the redirect lands on a live 200 page that IS in the
   sitemap (one hop, 301/308, not a chain).
2. If the redirect is correct → **no code change**; remove any internal links still pointing at
   the old URL (search the repo for the old slugs), then in GSC mark validation done / ignore.
   This bucket will sit at 12 forever and that is fine.
3. If any redirect lands on the homepage or a wrong page → point it at the true replacement
   in `next.config.js` redirects.
4. Likely members: pre-rename slugs (the repo history will show old article slugs) and
   `http://` / `www` variants (both verified to 308 correctly — those are healthy).

### C.2 Not found (404) — 13 pages, validation Started 🔄

**What it means:** Google remembers URLs that no longer exist. Harmless if they never had
traffic/links; wasteful if something still links to them.

**Fix:**
1. Export the 13 URLs. Split them:
   - **Has a direct replacement** (article was renamed/merged) → add a 301 in
     `next.config.js` `redirects()` to the new slug.
   - **Genuinely gone, no replacement** → leave as 404, or better return **410 Gone**
     (Next.js: keep 404; 410 needs a custom route — only worth it if the URL keeps getting
     re-crawled for months).
2. Check nothing internal links to them: `grep -r "<old-slug>" app/ components/ lib/` for each.
3. Check for external backlinks to these URLs (Ahrefs Site Explorer → Best by links → 404).
   Any 404 with real backlinks MUST get a 301 to the closest live page — that link equity is
   otherwise lost.

### C.3 Soft 404 — 3 pages, Not Started ⏳

**What it means:** the page returns 200 but Google judged the content "empty/not found"-like.
Typical causes here: a calculator or category page rendering with no results, or a thin
placeholder.

**Fix:**
1. Export the 3 URLs and open each in a browser.
2. If the page is real content → it likely rendered empty for Googlebot (JS-dependent content).
   Ensure the meaningful content is in the server-rendered HTML (these are Next.js static pages,
   so check the built HTML in `.next/server/app` contains the body text).
3. If the page genuinely has no content → either fill it or 301 it to its parent hub.
4. After fixing, GSC → validate fix.

### C.4 Redirect error — 2 pages, Not Started ⏳

**What it means:** Google followed a redirect and it broke — a loop, a chain that's too long
(>5 hops), or a redirect to an invalid/unreachable URL.

**Fix:**
1. Export the 2 URLs and trace each with `curl -sIL <url>` — count hops and look for loops.
2. Collapse any chain to a single 301 hop in `next.config.js`.
3. Common culprit: `http://www.` → `https://www.` → `https://` chains combined with a slug
   redirect (3+ hops). Make the old slug redirect directly to the final https non-www URL.

### C.5 Crawled — currently not indexed — 6 pages, validation FAILED ❌

The §2B inspection found only 2 such pages in the sitemap (both PMKMY, one now indexed —
see §B). The GSC bucket says 6 → export the list; the other ~4 are likely non-sitemap URLs
(old slugs redirect-targets, or pages later removed). For any that ARE current sitemap pages:
Request Indexing after the internal-linking fixes get crawled; for old URLs, treat per C.1/C.2.

### C.6 Excluded by 'noindex' — 1 page ⏳

Almost certainly `/search` — which is **intentional** (Fixes 4+7 made it a clean, crawlable
noindex). Verify the URL in GSC; if it's `/search`, ignore this bucket permanently. If it's any
other page, that's a bug — find and remove the stray `noindex`.

### C.7 Alternate page with proper canonical tag — 2 pages ⏳

**Normal and healthy.** These are pages Google chose not to index because they correctly
canonicalise elsewhere (e.g. `www`/`http` variants of indexed pages). No action.

### C.8 Discovered — currently not indexed — 25 pages, validation PASSED ✅

Already fully covered by §2B error group 1 and fixed by Parts 9–11. Validation has passed;
the count will drain as Googlebot crawls. No further action beyond the Request Indexing list
in §B.

---

## D. Priority order for this revision

| Priority | Action | Type | Effort |
|---|---|---|---|
| **P0** | Request Indexing on 6 URLs (§B list) | GSC UI | 10 min |
| **P0** | Trigger Ahrefs re-crawl (project 10042735) | Ahrefs UI | 1 click |
| **P1** | Export the 4 error-bucket URL lists from GSC (redirect ×12, 404 ×13, soft-404 ×3, redirect-error ×2) | GSC UI | 5 min |
| **P1** | C.4 — fix the 2 redirect errors (loops/chains) | `next.config.js` | minutes |
| **P2** | C.2 — 301s for renamed 404 URLs; check backlinks first | `next.config.js` | ~30 min |
| **P2** | C.3 — fix the 3 soft 404s | per-page | ~30 min |
| **P3** | C.1 — verify the 12 redirects land correctly, clean stray internal links | audit | ~30 min |

## E. Verification (2–4 weeks out)

1. GSC → Pages: "Discovered — not indexed" should fall from 34; "URL unknown" from 10 → 0.
2. Ahrefs issue `d69246c2-225a-11ec-8456-06d2f2f613d8` count: 33 → near 0 after re-crawl.
3. `/maandhan/pmkmy-grievance-complaint-helpline` should flip to indexed after re-crawl of the
   Part-13 upgrade; if still out after 4 weeks, merge it into a combined PMKMY guide (per §2B).
4. Re-export GSC coverage and diff against `kisanstatus.com-Coverage-2026-08-17.zip`.

**Bottom line:** all code fixes from Revisions 1–3 are deployed and verified live; early index
movement is already visible (1 page indexed, 2 out of "unknown"). What remains is (a) six
Request-Indexing clicks, (b) the Ahrefs re-crawl, and (c) closing out the legacy-URL buckets in
§C — which need the per-bucket URL exports from GSC before any redirect is written.
